...
@database
def test_jdbc_multitable_consumer_origin_configuration_batches_from_result_set(sdc_builder, sdc_executor,
                                                                               per_batch_strategy, database):
    """Check working of attribute of batches_from_result_set for JDBC multi table consumer.
    ...
    try:
        ...
        for record in snapshot_contents:
            ...
            thread_number = header['jdbc.threadNumber']
            if thread_number not in threads:
...
def get_id_name_data_map(number_of_records, starting_point=1):
    return [{'id': '{}'.format(i),
             'name': '{}_{}'.format(get_random_string(string.ascii_lowercase, 6), i)}
            for i in range(starting_point, (starting_point+number_of_records))]


def create_table_and_insert_data(database, src_table_prefix, number_of_records, table_number):
    table_name = '{}_{}'.format(src_table_prefix, get_random_string(string.ascii_lowercase, 20))
    rows_in_table = get_id_name_data_map(number_of_records, (table_number * number_of_records + 1))
